...
static func from_dict(data: Dictionary) -> MatchRecord:
	...
	for pid_str in stats_dict:
		...
		ps.final_hp = d.get("final_hp", 0.0)
	...
	for snap_data in data.get("round_snapshots", []):
		...
		for sd in snap_data.get("player_states_after", []):
			...
			pss.hp = sd.get("hp", 0.0)
			pss.energy = sd.get("energy", 0)
			pss.has_shield = sd.get("has_shield", false)
			pss.paralyze_turns = sd.get("paralyze_turns", 0)
			pss.knockdown_turns = sd.get("knockdown_turns", 0)
			pss.is_alive = sd.get("is_alive", false)
			pss.bell_count = sd.get("bell_count", 0)
			pss.counter_stance = sd.get("counter_stance", false)
			pss.skill_disabled_turns = sd.get("skill_disabled_turns", 0)
			pss.gate_count = sd.get("gate_count", 0)
			pss.invincible_turns = sd.get("invincible_turns", 0)
			pss.burning = sd.get("burning", false)
			pss.berserker = sd.get("berserker", false)
			pss.consecutive_rounds = sd.get("consecutive_rounds", 0)
			pss.lost_skills = sd.get("lost_skills", [])
			pss.ftg_marks = sd.get("ftg_marks", 0)
			pss.ftg_marked_by = sd.get("ftg_marked_by", [])
			pss.nine_tails_stage = sd.get("nine_tails_stage", 0)
			pss.nine_tails_invincible = sd.get("nine_tails_invincible", false)
