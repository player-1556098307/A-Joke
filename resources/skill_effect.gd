...
enum EffectType {
	...
	## 斩魂祝福改为直接5点伤害（旧枚举保留以防.tres整数错位）
	PERCENT_DAMAGE,
	## ── 宙斯Boss专属 ──────────────────────────────────────────
	## 神大罚（宙斯限定技）：使目标HP降至1并麻痹3回合（直接设HP，不走吸收链）
	ZEUS_JUDGEMENT,
	## 变异军团（宙斯）：消耗1行动权召唤1个异种（value=方向 0=顺时针 1=逆时针）
	ZEUS_SUMMON_MUTANT,
	## 变异军团升级（宙斯二阶段）：消耗1行动权召唤1个克罗狄亚
	ZEUS_SUMMON_CLODIA,
	## ── 奥伯龙专属 ──────────────────────────────────────────
	## 夜之帷幕（准备阶段被动标记）：获得夜幕降临状态，持续到主动用梦之终结取消
	## 效果：每次准备阶段获得1气 + 本回合无法造成伤害（伤害归零，不取消夜幕）
	NIGHT_CURTAIN,
	## 梦之终结（结束阶段主动）：结束夜幕降临状态，选择任意玩家（含自己）下次伤害x2
	## 所有伤害类型均生效（普攻/技能/燃烧/狂战士/延迟伤害等），由 GameManager 处理
	DREAM_END,
	## 仲夏夜之梦（操作阶段主动）：条件需处于夜幕降临状态，消耗1气获得3护盾（不可叠加）
	## 不可叠加：若已有护盾则不累加，直接设为3（取较大值）
	MIDSUMMER_DREAM,
	## 于彼方点缀的梦之童话（5气限定技）：指定任意名其他玩家造成3伤+麻痹1回合
	## 自身免疫下次受到的攻击（防御性，持续到下次被攻击时触发）
	FAIRY_TALE,
	## ── 阿尔托莉雅·卡斯特专属 ──────────────────────────────────────
	## 乐园妖精（被动标记）：开局获得5个气
	FAIRY_PARADISE,
	## 巡礼（被动标记）：每次消耗气自动获得一个圣盾（全挡护盾，不可叠加）
	## 累计触发4次后升级为【圣剑锻造】，不再继续触发
	PILGRIMAGE,
	## 湖之加护（结束阶段被动标记）：施法者自己回合结束阶段选择任意玩家使其获得1气
	LAKE_BLESSING,
	## 圣剑锻造（操作阶段主动，3气）：选择一名其他玩家，使其无消耗释放一次任意需耗气的技能
	## 无视释放条件，若目标无可用技能则目标获得3气
	SWORD_FORGE,
}
